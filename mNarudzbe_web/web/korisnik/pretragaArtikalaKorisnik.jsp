                        ...
                        <div class="container" style="background-color:#8686BA; -webkit-border-radius: 36px; padding: 10px ; float:right ">
                            <center>
                                <form method="GET" name="forma" >
                                    <div class="row-fluid">
                                        <table style="width: 350px;" >
                                            <th colspan="2" style="text-decoration:underline;">PRETRAGA ARTIKLA</th>
                                            <br/>
                                            <tr><td></td>
                                                <td>
                                            ...
                                            <tr>
                                                <td>ID artikla</td>
                                                <td><input class="input-block-level" placeholder="ID Artikla" type="text" name="id_artikla" size="5" maxlength="5" value="<c:out value="${param.id_artikla}"/>"/></td>
                                            </tr>
                                            <tr>
                                                <td>Naziv artikla </td>
                                                <td><input class="input-block-level" placeholder="Naziv artikla" type="text" name="naziv_artikla" value="<c:out value="${param.naziv_artikla}"/>"/></td>
                                            </tr>
                                ...
                                <c:if test="${pageContext.request.method=='GET'}">
                                    <c:choose>
                                        <c:when  test="${(param.id_artikla!='') && (param.naziv_artikla=='') }">
                                            <sql:query dataSource="${con}" var="forma">SELECT * from mnarudzbe_${korisnik.username}_artikli WHERE ID_Artikla='<c:out value="${param.id_artikla}"/>'</sql:query>

                                        </c:when>
                                        <c:when  test="${(param.id_artikla=='') && (param.naziv_artikla!='')}">
                                            <sql:query dataSource="${con}" var="forma">SELECT * from mnarudzbe_${korisnik.username}_artikli WHERE Naziv_Artikla='<c:out value="${param.naziv_artikla}"/>'</sql:query> 
                                        </c:when>
                                        <c:when  test="${(param.id_artikla!='') && (param.naziv_artikla!='')}">
                                            <sql:query dataSource="${con}" var="forma">SELECT * from mnarudzbe_${korisnik.username}_artikli WHERE Naziv_Artikla='<c:out value="${param.naziv_artikla}"/>' AND ID_Artikla='<c:out value="${param.id_artikla}"/>'</sql:query>
                                        </c:when> 
                                        <c:otherwise>
                                            <sql:query dataSource="${con}" var="forma">SELECT * from mnarudzbe_${korisnik.username}_artikli</sql:query>
                                        </c:otherwise>

                                    </c:choose>
                                    <display:table name="${forma.rows}" pagesize="${param.redaka}" style="border:1px solid #666; text-align:center; width:600px">   
                                        <display:column property="ID_Artikla" title="ID artikla" style="border:1px solid #666"/>
                                        <display:column property="Naziv_Artikla" title="Naziv artikla" style="border:1px solid #666"/>
                                        <display:column property="Skupina_Artikla" title="Skupina artikla" style="border:1px solid #666"/>
                                        <display:column property="Cijena_Artikla" title="Cijena artikla (kn)" style="border:1px solid #666"/>
                                    </display:table>       
                                </c:if>    
                            </center>
                        </div>
